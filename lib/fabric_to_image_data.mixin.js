  ...
  var FabricToImageData = function (_module) {
    ...
    if (fabric.util.toImageData) {
      ...
      fabric.util.toImageData = function (canvasEl, sx, sy, sw, sh) {
        ...
        sw = sw || 1;
        sh = sh || 1;
        return canvasEl.getContext('2d').getImageData(sx, sy, sw, sh);
      };
    }
     ...
     * @return {ImageData} Returns a ImageData
     */
    if (fabric.Object.prototype.toImageData) {
      fabric.warn('fabric.Object.prototype.toImageData is already defined');
    } else {
      fabric.Object.prototype.toImageData = function (options) {
        options || (options = {});
        ...
        if (options.fixedLength) {
          var ratio = Math.min((options.fixedLength - 1) / length, 1);
          this.set('scaleX', this.scaleX * ratio);
          this.set('scaleY', this.scaleY * ratio);
        }

        if (options.fixedStrokeWidth) {
          origParams.strokeWidth = this.strokeWidth;
          this.set('strokeWidth', options.fixedStrokeWidth / ratio);
        }
